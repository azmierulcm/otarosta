...
const AchievementBadge = ({ definition, earned, earnedAt }: BadgeProps) => {
  ...
  const tierStyles: Record<AchievementTier, string> = {
    'common': 'bg-gray-100 border-gray-200 text-gray-400',
    ...
    'easter-egg': 'bg-gray-900 border-gray-800 text-gray-600'
  };

  const activeStyle = earned ? tierStyles[definition.tier] : 'bg-gray-50 border-gray-100 text-gray-200 opacity-40';
  ...
  return (
    ...
    >
      ...
      <div className="text-center">
        ...
        <h4 className={`text-sm font-black leading-tight ${earned ? 'text-gray-900' : 'text-gray-300'}`}>
          {definition.name}
        </h4>
        
        {earned && definition.rarity_pct && (
          <p className="text-[9px] font-bold text-rausch mt-2 uppercase tracking-tight">
